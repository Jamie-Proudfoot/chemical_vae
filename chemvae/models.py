...
# from tensorflow.compat.v1.keras.layers import Dense, Flatten, RepeatVector, Dropout
from keras.layers import Convolution1D
# from tensorflow.compat.v1.keras.layers import Convolution1D
...
def variational_layers(z_mean, enc, kl_loss_var, params):
    ...
    epsilon = K.random_normal_variable(
        shape=(params['batch_size'], params['hidden_dim']),
        mean=0.0,
        scale=1.0
    ) # fixed epsilon to match the original implementation

    def sampling(args):
        z_mean, z_log_var = args

        # epsilon = K.random.normal(shape=(params['batch_size'], params['hidden_dim']),
        #                                    mean=0., stddev=1.)
        # insert kl loss here
